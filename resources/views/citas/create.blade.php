...
@section('link'){{url('citas/create')}}@endsection
@section('link2','Listado de Citas ')
@section('content')


        <div class="row align-items-center">
            <div class="col"><h2 class="text-center">DOCTOR: {{$doctor->nombres}} {{$doctor->apellidoPaterno}} {{$doctor->apellidoMaterno}} </h2></div>
            <div class="col">
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{url($doctor->id.'/'.$j->fecha.'/show')}}" method="POST">
                                    @csrf
                                <button type="submit" class= "btn btn-outline-secondary">{{\Carbon\Carbon::parse($j->fecha)->format('d-m-Y')}}</button>
                                </form>
                            </td>
